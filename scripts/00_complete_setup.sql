...
RDW Open Data Sources (all 100% loaded):
  ...
  - 8ys7-d773: Gekentekende voertuigen brandstof (50k sample of 16.7M)
  - ygq4-hh5q: Parkeeradres (3,382 records with type=F)
  - m9d7-ebf2: Gekentekende voertuigen (50k sample) - Registration dates for time analysis
=============================================================================
*/
...
CREATE WAREHOUSE IF NOT EXISTS PON_ANALYTICS_WH
...
CREATE OR REPLACE FUNCTION PON_EV_LAB.RAW.FETCH_RDW_DATA(dataset_id STRING, row_limit INT, row_offset INT)
...
CREATE OR REPLACE FUNCTION PON_EV_LAB.RAW.FETCH_RDW_DATA(dataset_id STRING, row_limit INT, row_offset INT, order_col STRING)
RETURNS VARIANT LANGUAGE PYTHON RUNTIME_VERSION = '3.11' PACKAGES = ('requests')
EXTERNAL_ACCESS_INTEGRATIONS = (rdw_api_access) HANDLER = 'fetch_data'
AS $$
import requests
...
api_data AS (SELECT PON_EV_LAB.RAW.FETCH_RDW_DATA('8wbe-pu7d', 1000, offset_val) AS data FROM offsets)
...
WITH api_data AS (SELECT PON_EV_LAB.RAW.FETCH_RDW_DATA('b3us-f26s', 10000, 0) AS data)
...
WITH offsets AS (SELECT (ROW_NUMBER() OVER (ORDER BY SEQ4()) - 1) * 1000 AS offset_val FROM TABLE(GENERATOR(ROWCOUNT => 50))),
api_data AS (SELECT PON_EV_LAB.RAW.FETCH_RDW_DATA('8ys7-d773', 1000, offset_val, 'kenteken') AS data FROM offsets)
...
WITH api_data AS (SELECT PON_EV_LAB.RAW.FETCH_RDW_DATA('ygq4-hh5q', 10000, 0) AS data)
...
WITH offsets AS (SELECT (ROW_NUMBER() OVER (ORDER BY SEQ4()) - 1) * 1000 AS offset_val FROM TABLE(GENERATOR(ROWCOUNT => 50))),
api_data AS (SELECT PON_EV_LAB.RAW.FETCH_RDW_DATA('m9d7-ebf2', 1000, offset_val, 'kenteken') AS data FROM offsets)
SELECT f.value:kenteken::STRING, f.value:datum_eerste_tenaamstelling_in_nederland::STRING,
       f.value:merk::STRING, f.value:handelsbenaming::STRING, f.value:voertuigsoort::STRING, f.value
FROM api_data, LATERAL FLATTEN(input => data) f;
...
CREATE OR REPLACE DYNAMIC TABLE CURATED.EV_BY_REGION
    TARGET_LAG = '1 hour' WAREHOUSE = PON_ANALYTICS_WH
...
CREATE OR REPLACE SHARE PON_DEALER_SHARE COMMENT = 'EV analytics for Pon dealer network - live data, no copies';
...
-- PON_OEM_SHARE is conceptual only (see lab guide Section 5.6)
-- CREATE OR REPLACE SHARE PON_OEM_SHARE COMMENT = 'EV market data for OEM partners (anonymized)';
...
SELECT 'VEHICLES_BY_POSTCODE_RAW' AS table_name, COUNT(*) AS row_count, 46645 AS expected FROM RAW.VEHICLES_BY_POSTCODE_RAW
UNION ALL SELECT 'CHARGING_CAPACITY_RAW', COUNT(*), 3139 FROM RAW.CHARGING_CAPACITY_RAW
UNION ALL SELECT 'VEHICLES_FUEL_RAW', COUNT(*), 50000 FROM RAW.VEHICLES_FUEL_RAW
UNION ALL SELECT 'PARKING_ADDRESS_RAW', COUNT(*), 3382 FROM RAW.PARKING_ADDRESS_RAW
UNION ALL SELECT 'VEHICLES_RAW', COUNT(*), 50000 FROM RAW.VEHICLES_RAW;

SELECT 'KENTEKEN OVERLAP' AS check_name, COUNT(DISTINCT v.kenteken) AS overlap_count
FROM RAW.VEHICLES_RAW v JOIN RAW.VEHICLES_FUEL_RAW f ON v.kenteken = f.kenteken;
