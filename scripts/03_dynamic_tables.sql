...
-- =============================================================================
-- TALKING POINT: DCM Projects (Database Change Management)
-- =============================================================================
/*
STATUS: Private Preview (March 2026) - Available on AWS, Azure, GCP

WHAT IS IT:
Everything you just built manually (database, schemas, Dynamic Tables,
warehouses, roles, grants) can be defined DECLARATIVELY as code.
You describe the desired state, Snowflake figures out what to change.
Think Terraform, but native to Snowflake with full pipeline support.
...
DEFINE DYNAMIC TABLE {{env}}_PON_EV_LAB.ANALYTICS.EV_INFRASTRUCTURE_CORRELATION
    TARGET_LAG = '1 hour'
    WAREHOUSE = {{env}}_PON_ANALYTICS_WH
AS
    SELECT e.postal_area, e.electric_vehicles, e.ev_percentage,
        COALESCE(l.total_laadpalen, 0) AS charging_points
    FROM {{env}}_PON_EV_LAB.CURATED.EV_BY_REGION e
    LEFT JOIN (
        SELECT LEFT(postcode, 2) AS postal_area, SUM(aantal) AS total_laadpalen
        FROM {{env}}_PON_EV_LAB.ANALYTICS.LAADPALEN_PER_POSTCODE
        GROUP BY 1
    ) l ON e.postal_area = l.postal_area;

GRANT SELECT ON ALL DYNAMIC TABLES IN SCHEMA {{env}}_PON_EV_LAB.ANALYTICS
    TO ROLE {{env}}_PON_ANALYST;
...
--     FROM 'snow://workspace/...';
--
-- After deploy, manage the full pipeline lifecycle:
--
...
--     PROD_PON_EV_LAB.ANALYTICS.EV_INFRASTRUCTURE_CORRELATION
--     LIMIT 100;

COMPETITIVE COMPARISON:
---------------------------------------------------------------------------
| Capability                  | Snowflake DCM      | Databricks          | MS Fabric           |
|-----------------------------|--------------------|---------------------|---------------------|
| Declarative definitions     | Native DEFINE      | Terraform (3rd pty) | ARM/Bicep templates |
| Pipeline management (DTs)   | Built-in           | N/A (no DT equiv)   | N/A                 |
| Data quality in deployment  | TEST ALL (native)  | Great Expectations  | No native equiv     |
| Multi-env with one codebase | Jinja templating   | Manual configs      | Param files         |
| Plan before deploy          | PLAN command        | terraform plan      | What-if analysis    |
| Refresh all pipelines       | REFRESH ALL         | Manual job triggers | Manual              |
| Preview data pre-deploy     | PREVIEW command     | Not available       | Not available       |
| Version-controlled          | Git-native          | Git + external tool | Git + external tool |
| Object scope                | All SF objects      | Limited to UC       | ARM-scoped          |

KEY DIFFERENTIATOR vs dbt:
- dbt manages transformations only (SELECT statements)
- DCM manages EVERYTHING: databases, schemas, warehouses, roles, grants,
  Dynamic Tables, tasks, streams, AND data quality -- all in one project
- No external orchestrator needed -- Snowflake handles dependency ordering
- Native PLAN/DEPLOY/TEST/REFRESH lifecycle, no 3rd-party CI/CD required

WHY THIS MATTERS FOR PON:
- One tool for infrastructure + pipelines + governance + quality gates
- Deploy identical pipelines across dev/staging/prod with Jinja variables
- Data quality expectations as code, tested before every production deploy
- Full audit trail of every deployment version
- No Terraform, no dbt, no Airflow -- just Snowflake
=============================================================================
*/
